...
export async function GET() {
    ...
    if (isDemoUser) {
        ...
        if (!hasDemoTrips) {
            ...
            try {
                await Promise.all([
                    Trip.updateOne({ title: "Goa March 2026", ownerId: userId }, { $setOnInsert: goaData }, { upsert: true }),
                    Trip.updateOne({ title: "Manali Adventure", ownerId: userId }, { $setOnInsert: manaliData }, { upsert: true }),
                ]);
            } catch (seedErr) {
                console.log("Demo seed collision (expected in StrictMode):", seedErr);
            }

            // Re-fetch now that they exist (whether created by this request or the concurrent one)
