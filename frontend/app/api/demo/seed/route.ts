...
export async function POST(req: Request) {
    try {
        ...
        // Run both upserts in parallel — single round trip each instead of 4
        await Promise.all([
            Trip.updateOne(
                { title: "Goa March 2026", ownerId: userId },
                { $setOnInsert: goaData },
                { upsert: true }
            ),
            Trip.updateOne(
                { title: "Manali Adventure", ownerId: userId },
                { $setOnInsert: manaliData },
                { upsert: true }
            ),
        ]);
